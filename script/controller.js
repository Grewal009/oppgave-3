function runGasStation() {
  if (fuelAmount < 50) {
    // if fuelAmount is less than 10 show the gas station
    showGasStation = true;
  ...
  if (fuelAmount > 50) {
    // if fuelAmount is greater than 10 don't show the gas station
...
function startEvent(eventIndex) {
  // eventIndex == newEventID sent to function, just different name due to index value usage
...
function runPickup() {
  ...
  encounterDialogue =
    "You found " + currentEvent.name + ", would you like to pick it up?";
...
function resolvePickup(chosenOption) {
  ...
  if (chosenOption == 1) {
    ...
    encounterDialogue =
      "You picked up:" + currentEvent.name + ", you gained " + carCoolness;
...
function resolveEncounter(chosenOption) {
  ...
  if (
    chosenOption == currentEvent.correctResponse &&
    carCoolness >= currentEvent.requiredCoolness
  ) {
...
function UpdateCoolness() {
  ...
  document.getElementById("coolnessBar").style.width = `${
    (carCoolness / coolness_max) * 100
  }%`;
}
...
function restart() {
  ...
  //reset model here (initial value) or use location.reload();
